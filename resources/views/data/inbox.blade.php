...
<script>
    document.getElementById('sendReply').addEventListener('click', function() {
        ...
        const sessionId = document.getElementById('sessionInput').value; // Asumsi session_id disimpan di sini
        const from = document.getElementById('fromInput').value; // Asumsi 'from' disimpan di sini

        if (replyInput.trim() !== '') {
            // Buat query string untuk GET request
            const url =
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    // Tampilkan pesan balasan yang baru
    ...
    function replyMessage(from) {
        ...
        fetch(`${messagesApiUrl}`)
            ...
            .then(data => {
                ...
                const sortedMessages = filteredMessages.sort((a, b) => new Date(a[1].create_at) - new Date(b[1].create_at));
