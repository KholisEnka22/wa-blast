    ...
    <div class="card mb-4">
        ...
        <div class="card" style="padding: 10px">
            ...
            <div class="d-flex justify-content-end mb-3">
                ...
                <button id="myButton" class="btn btn-icon btn-primary ms-2" data-bs-toggle="tooltip" data-bs-offset="0,4"
                    data-bs-placement="right" data-bs-html="true" title="<span>Tambahkan Server</span>">
                        ...
                        @if (count($server) > 0)
                            @foreach ($server as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->count }}</td>
                                    <td>
                                        <form action="{{ route('setting.toggle-server', ['id' => $item->id]) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit"
                                                class="btn badge {{ $item->status === 'Active' ? 'bg-label-success' : 'bg-label-danger' }} me-1"
                                                onclick="return confirm('Apakah Anda yakin ingin mengubah status server?')">
                                    ...
                                    <td>
                                        <form action="{{ route('server.destroy', ['id' => $item->id]) }}" method="POST">
                                            ...
                                            <button type="submit"
                                                ...
                                                onclick="return confirm('Apakah Anda yakin ingin menghapus server?')">
